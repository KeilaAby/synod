...
export function BaptemeDialog({
  ...
  const gradeParDefaut =
    grades.find((g) => g.libelle.toLocaleLowerCase('fr') === 'croyant')?.id ?? grades[0]?.id;
  ...
  return (
    <>
      ...
      <Dialog open={ouvert} onOpenChange={(v) => (v ? setOuvert(true) : fermer())}>
        <DialogContent className="max-h-[92vh] w-[min(96vw,64rem)] overflow-y-auto sm:max-w-4xl">
          <DialogHeader>
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <Select
                            ...
                            disabled={!egliseChoisie || cellulesDisponibles.length === 0}
                          >
                            <SelectTrigger {...aria} className="h-10 w-full">
