 ...
 * la cle de service (voir `lib/storage/supabase-adapter.ts`) : le
 * `requirePermission` ci-dessous n'est pas une commodite d'affichage doublee
 * par la RLS, c'est le controle d'acces lui-meme. Ne jamais appeler `storage()`
 * sans l'avoir precede.
 */
