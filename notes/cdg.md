...
| **EF-ADM-03** | Restreindre une habilitation à une **sous-structure** du périmètre du compte (ex. `finance.create` limité à une seule paroisse du district). *(Implémenté puis retiré le 23 août 2026 : le sélecteur de portée par droit, testé et fonctionnel, a surchargé visuellement le formulaire de compte — treize droits actifs affichaient treize sélecteurs, même en mode discret. L'utilisateur a demandé de revenir en arrière : une habilitation reste bornée à l'entité de rattachement du compte, **entière** — jamais restreinte à une sous-structure. Le besoin d'atteindre une entité enfant reste couvert par RG-25 (un droit `DESCENDANTE` couvre déjà l'entité et tout son sous-arbre) combiné à `sans_acces_application` : « si une entité enfant n'a pas accès à l'application, son entité parent peut saisir pour elle, dans la limite de l'habilitation d'un compte au sein de son parent. » Code : `git revert` de `f85775c`, `44db5f9`, `54d2d49` — voir `notes/todos.md` §5.)* | **Must**, décliné par décision explicite |
