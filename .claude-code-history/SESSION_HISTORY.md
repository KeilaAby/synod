...
## 22 août 2026 (suite) — Migration `0073` confirmée, puis la portée par droit dans l'octroi

L'utilisateur a appliqué la migration `0073` (en-tête par entité) et testé.
Reprise ensuite de `notes/todos.md` §5 : « Portée par droit dans l'octroi. »

Exploration du mécanisme existant avant d'écrire une ligne :
`user_permissions.scope_entity_id` existe depuis la TOUTE PREMIÈRE migration
(`0005`), et `has_perm()` (SQL) comme `peut()`/`porteeCouvre()`
(`lib/domain/permissions.ts`) savent déjà le lire — seul l'écran n'avait
jamais posé autre chose que l'entité de rattachement du compte, dans les
deux Server Actions `creerCompte`/`modifierCompte`
(`lib/actions/comptes.ts`). Le formulaire (`compte-dialog.tsx`) le
promettait même déjà, sans le tenir : « une portée plus fine se règle
ensuite ».

**Question posée avant d'écrire : un sélecteur de portée par droit coché, ou
une portée globale pour tout l'octroi ?** La base permet une portée
DIFFÉRENTE par ligne de `user_permissions`, et un compte porte souvent une
vingtaine de droits actifs. Réponse — un sélecteur par droit, malgré le coût
à l'écran, pour ne pas perdre cette flexibilité que le schéma porte depuis
l'origine.

`SelecteurHabilitations` (`components/administration/selecteur-habilitations.tsx`)
reçoit un prop `portee` **optionnel** : absent pour `reglages-profils.tsx`
(un PROFIL est un jeu de clés commun à toute l'organisation, sans entité à
restreindre — lui passer `portee` n'aurait aucun sens), fourni par
`compte-dialog.tsx` pour l'écran de compte. Un `EntityPicker` compact
apparaît sous chaque droit **actif** (restreindre un droit qu'on n'accorde
pas ne voudrait rien dire), borné au sous-arbre de l'entité de rattachement
du compte, l'entité elle-même exclue (la resélectionner équivaut à
l'absence de restriction, `null`).

**`resoudrePortee`, nouvelle fonction PURE dans `lib/domain/permissions.ts`,
testée (6 cas dans `tests/unit/permissions.test.ts`).** Elle NORMALISE en
`null` quand l'entité choisie est celle du compte lui-même — écrire l'id
explicite quand même figerait la portée si le compte était un jour
re-rattaché, là où `null` suit toujours son entité courante. Elle REFUSE une
entité hors du sous-arbre du compte bénéficiaire (une sœur, un ancêtre) :
« restreint à cette sous-structure », le commentaire d'origine de `0005`,
devient une vraie contrainte plutôt qu'une promesse de commentaire.

**Deux contrôles distincts, tous deux nécessaires.** `resoudrePortee` borne
la portée au sous-arbre du COMPTE bénéficiaire ; `peutDeleguer` (RG-24,
totalement inchangée) vérifie ensuite que cette même portée reste dans ce
que le DÉLÉGANT détient lui-même. Un délégant peut détenir un droit
largement et vouloir l'accorder plus étroit que ce qu'il détient ; le
compte bénéficiaire, lui, ne doit jamais recevoir une portée qui déborde de
son propre rattachement — aucun des deux contrôles ne remplace l'autre.

Défaut trouvé en chemin, troisième occurrence de la même classe cette
session (`promotionDuCroyant`, `organisation_settings.logo_key`) :
`lib/data/comptes.ts` sélectionnait déjà `scope_entity_id` par
habilitation — `comptes-client.tsx` le jetait en reconstruisant
`accordees` comme un simple tableau de clés avant de le passer au pop-up.
